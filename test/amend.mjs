// Suite N — Amend (append-only follow-ups). The user sends a nudge, then realises
// they want to add one more thought to the SAME nudge. Proves: an OPEN nudge takes
...
// History "+ amend" UI round-trips. Own bridge on side port 4788, own store.
...
try {
  ...
  {
    const id = await addNudge('The card needs more spacing.')
    const r = await post(`/comments/${id}/amend`, { text: 'and the title one size smaller' })
    if (r.status !== 200) fail(`N1: amend returned ${r.status}`)
    const pin = readStore().pins.find(p => p.id === id)
    if (pin.text !== 'The card needs more spacing.') fail('N1: original text was mutated')
    if (pin.amendments?.length !== 1 || pin.amendments[0].text !== 'and the title one size smaller') fail(`N1: amendment not stored: ${JSON.stringify(pin.amendments)}`)
    if (!pin.amendments[0].at) fail('N1: amendment missing timestamp')
    const md = inbox(id)
    if (!md.includes('The card needs more spacing.') || !md.includes('Amendment') || !md.includes('and the title one size smaller')) fail(`N1: inbox mirror missing original+amendment:\n${md}`)
  ...
  {
    const id = await addNudge('First thought.')
    await post(`/comments/${id}/amend`, { text: 'second' })
    await post(`/comments/${id}/amend`, { text: 'third' })
    const empty = await post(`/comments/${id}/amend`, { text: '   ' })
    if (empty.status !== 400) fail(`N2: empty amend should be 400, got ${empty.status}`)
    const pin = readStore().pins.find(p => p.id === id)
    if (pin.amendments.map(a => a.text).join(',') !== 'second,third') fail(`N2: order/content wrong: ${JSON.stringify(pin.amendments)}`)
  ...
  {
    const id = await addNudge('Almost done.')
    await post(`/comments/${id}/resolve`)
    const r = await post(`/comments/${id}/amend`, { text: 'too late' })
  ...
  // ---------- N4: a REAL watcher re-wakes on an amendment (the payoff) ----------
  {
    ...
    const id = await addNudge('Please make the banner wider.')
    for (let i = 0; i < 30 && !out.includes(`New nudge ${id}`); i++) await sleep(100)
    if (!out.includes(`New nudge ${id}`)) fail(`N4: watcher did not wake on the fresh nudge:\n${out}`)
    await post(`/comments/${id}/amend`, { text: 'and with more presence' })
    for (let i = 0; i < 30 && !out.includes(`Nudge ${id} amended`); i++) await sleep(100)
    if (!out.includes(`Nudge ${id} amended: and with more presence`)) fail(`N4: watcher did not re-wake on the amendment:\n${out}`)
