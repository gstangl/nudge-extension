...
| G-8 | A FOREIGN agent (CI, unrelated task) saw Nudge context and related to the owner's session („Die neuen Nudges gehören der suite-e-Session") — the opt-in FENCE (0.10.2) stopped ARMING but not INFORMING; the UserPromptSubmit + SessionStart hooks injected into EVERY session as long as a global store existed | immanent gate: the context hook reveals Nudge ONLY to sessions whose id is in the bridge roster (armed via /nudge); SessionStart injects nothing. No roster membership → total silence | ✅ Suite G (4 legs); hooks port/store-configurable for hermetic testing (fixed 0.16.5) |
